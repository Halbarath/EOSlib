...
def innerloopbody(rho,T, iMatX, iMatY, X, Y, P_min, P_max):
...
if __name__ == '__main__':
    ...
    rho_axis = np.logspace(-1,2,151)
    T_axis = np.logspace(2,5,151)

    results = Parallel(n_jobs=32)(delayed(innerloopbody)(rho,T, iMatX, iMatY, X, Y, P_min, P_max) for T in T_axis for rho in rho_axis)
    f = open(filename, "w")
